...
class MedDetailViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "toListViewFromSave" {
            ...
            if (fromDatePicker.date <= toDatePicker.date) && nameTextField.text != "" && nicknameTextField.text != "" {
                ...
                ReminderHelperClass.scheduleNotification(for: (User.current.medicineArray?[self.catchIndexPath!])!)
                ...
                deleteNotificationForMedicine(theMedicineToBeDeleted: copyMedicineObjForDeleteNotifications, onCompletion: { () in
                })
            }
            else {
    ...
    func deleteNotificationForMedicine(theMedicineToBeDeleted: [String: String], onCompletion: @escaping () -> Void) {
    
        UNUserNotificationCenter.current().getPendingNotificationRequests { (notificationRequests) in
            //identifiers that will be deleted
            var identifiers: [String] = []
            
            //start matcjing to check all the identifier that is in the firebase being deleted from the notification center
            for notification: UNNotificationRequest in notificationRequests {
                
                for (key,_) in (theMedicineToBeDeleted) {
                    
                    if notification.identifier == key {
                        //append to array and to be deleted
